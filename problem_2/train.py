...
def sentiment_features(reviews):
    ...
    pos_counts = np.zeros(len(reviews))
    neg_counts = np.zeros(len(reviews))
    for i, review in enumerate(reviews):
        sentiment_scores = sia.polarity_scores(review)
        for word in pos_words:
            if word in review:
                pos_counts[i] += sentiment_scores["pos"]
        for word in neg_words:
            if word in review:
                neg_counts[i] += sentiment_scores["neg"]

    # Calculate relative frequency of positive and negative words for each review
    pos_freq = pos_counts / np.sum(pos_counts + neg_counts)
    neg_freq = neg_counts / np.sum(pos_counts + neg_counts)
    df = pd.DataFrame(
        {
            "review": reviews,
            "pos_freq": pos_freq,
...
def process_embeddering(nd_arr):
    embedding_list = list()
    dims = [f"dim_{i+1}" for i in range(len(nd_arr[0]))]
    for embedding in nd_arr:
        _embed = embedding[:]
        embedding_list.append(_embed)
    return pd.DataFrame(data=embedding_list, columns=dims)
...
def process_labels(labels):
    ...
    for label in labels:
        if label == "Positive":
            ...
            _labels.append(0)
    return pd.DataFrame({"targets": _labels})


# with open("./data/embeddings_bert-base-large.pkl", "rb") as f:
#     arch = pickle.load(f)

# label = arch["label"]
# raw_text = arch["raw_text"]
# cleanned_text = arch["cleanned_text"]
# embeddings = arch["embeddings"]
# checkpoint = arch["checkpoint"]

# df1 = sentiment_features(reviews=cleanned_text)
# df2 = process_embeddering(nd_arr=embeddings)
# df3 = process_labels(labels=label)


# final_df = pd.concat([df1, df2, df3], axis=1)
# final_df.to_csv("./data/master_data_bert-base-large.csv", index=False)


def cluster_preds(model, valid_dl, epoch: int):
    with torch.no_grad():
        model.eval()
        ...
        for data, label in valid_dl:
            ...
            preds.append(model_preds.numpy().flatten())
        ...
        PCA(n_components=2)
        kmeans = KMeans(2)
        features_transformed = umap.fit_transform(features)
        cluster_labels = kmeans.fit_predict(features_transformed)
        df = pd.DataFrame(
            {
                "X0": features_transformed[:, 0],
                "X1": features_transformed[:, 1],
                "True Label": labels.flatten(),
                "Predicted Label": cluster_labels,
            }
        )

        # Plot and save the figure
        plt.figure(figsize=(8, 6))
        palette = sns.color_palette("pastel", np.unique(cluster_labels).shape[0])
        sns.scatterplot(x="X0", y="X1", data=df, hue="Predicted Label", palette=palette)
        handles, labels = plt.gca().get_legend_handles_labels()
        plt.legend(handles, labels, loc="lower right")
        plt.title(f"Validation Data (Epoch {epoch+1})")
        plt.savefig(f"./validation_clusters/validation_epoch_{epoch+1}.png")
        plt.close()
...
master_data = pd.read_csv("./data/master_data_bert-base-large.csv")
